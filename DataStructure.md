...
算法(algorithm)：特定问题的求解步骤，它是指令的有限序列
...
一般情况下，可以通过考察算法所对应的程序中的**程序步数**来衡量算法的时间复杂度
...
如果存在两个正常数 $$c$$ 和 $$n_0$$ ，使得对所有的自然数 $$n$$ ，当 $$n>n_0$$ ,有 $$f(n)≤cg(n)$$ ，则称 $$f(n)=O(g(n))$$ ( $$g(n)$$ 是 $$f(n)$$ 的渐近上界)

定理：如果 $$f(n)= a_mn^m+a_{m-1}n^{m-1}+...+a_1n+a_0$$ 是 $$m$$ 次多项式，则 $$f(n)=O(n^3)$$

例如，设 $$T(n)=3.6n^3+2.5n^2+2.8$$ 则由于 $$T(n)≤ 20n^3$$ ，根据大O记号的定义容易证明  $$T(n)= O(n^3)$$
...
- 很多情况下，可以通过考察一个算法中的关键操作（关键操作被认为是一个执行次数最多的程序步）的执行次数来计算算法的渐近时间复杂度
- 有时也需要同时考虑几个关键操作，以反映算法的执行时间

因此，上面的E.g(1)的时间复杂度为 $$O(n)$$
...
例如，分别为包含100个元素的两个数组相加，与分别为包含10个元素的两个数组相加，所需的存储空间显然是不同的
...
数据： $$n$$ 个元素的线性序列 $$(a_0,a_1.….,a_{n-1})$$ ，其最大允许长度为maxSize，且 $$0≤n<maxSize$$ 
...
类型变量一旦定义，一**次性申请连续存储空间**用于存放指定数量的数据元素

不同点：

数组的元素具有**相同的数据类型**，而结构的成员(域)可以是不同类型数组元素用下标（index）标识，而结构成员由域名（field name）引用
...
数组类型采用顺序方式存储：数组中的元素按照**一定次序**顺序存放在一个**连续存储空间**

计算机中的存储空间是一维的，一维数组元素**可直接映射到存储空间中**

设给长度为 $$n$$ 的一维数组 $$a$$ 所分配的存储块的起始地址是 $$Loc(a[0])$$ ，若已知 $$a$$ 的每个元素占 $$k$$ 个存储单元，则下标为 $$i$$ 的数组元素 $$a[i]$$ 的存放地址：
...
数组类型采用顺序方式存储:数组中的元素按照一定次序顺序存放在一个连续存储空间
...
行优先顺序的地址计算二维数组 $$a[m][n]$$ ，已知每个数组元素占 $$k$$ 个存储单元， $$a[0][0]$$ 的存储地址是 $$loc(a[0][0])$$ ，则数组元素 $$a[i][j]$$ 的存储地址 $$loc(a[i][j])$$ 为：
...
- $$Loc(a[0][0])$$ 被称为基地址，它是存储数组的存储空间的起始地址
...
- 元素存储位置是其下标的线性函数，通过计算地址可实现对元素的随机存取
- 顺序存储一般借助数组来实现
- 数组名本身存储了指针值，其保存数组的首地址
- C语言中数组是**不允许整体赋值**的
- C语言中对数组下标超出正常范围的访问并**无合法性检查**，例如对长度为5的一维数组a，不合法的访问a[-3]，a[7]
- 数组是静态数据结构，其存储空间的大小需具体确定，并预先分配，一旦分配则**难以扩充**
...
在 $$n \times n$$ 的矩阵 $$A$$ 中，若 $$a_{[i][j]}=a_{[j][i]}$$ 则称其为 $$n$$ 阶对称矩阵

只存储上三角(或下三角)中的元素(包括对角线上的元素)

$$n \times n$$ 的对称矩阵，需要 $$\frac{n(n+1)}{2}$$ 个元素的存储空间，提高了存储效率

![](./image/sz5.png)

**对称矩阵的顺序存储**

以行优先顺序在一维数组B中存储下三角元素

存储位置公式为：

$$
index(a[i][j])=\begin{cases}
\frac{i(i+1)}{2}+j \ge i ≥ j \\
\frac{j(j+1)}{2}+i \ge j ≥ i
\end{cases}
$$
...
- 主对角以下元素全为0的方阵称为上三角矩阵
- 主对角以上元素全为0的方阵称为下三角矩阵
...
$$
index(a[i][j])=\begin{cases}
\frac{i(i+1)}{2}+j \ge 下三角矩阵 \\
\frac{j(j+1)}{2}+i \ge 上三角矩阵
\end{cases}
$$
...
- 当矩阵的稠密度很小，即包含**大量零元素**的矩阵称为**稀疏矩阵**
...
**CreateMatrix(M, rows, cols)**：构造运算: 构造一个 $$rows \times cols$$ 的空稀疏矩阵

**Clear(M)**：清除运算:清除稀疏矩阵中的所有非零元素

**Transpose(M)**：转置运算:返回稀疏矩阵 $$M$$ 的转置矩阵

}

非零元的位置分布没有规律，**需连同其位置信息一起存储，否则无法实现无损解压**

将稀疏矩阵中非零元 $$a_{ij}$$ 以三元组 $$<i,j,a_{ij}>$$ 表示
...
采用二维数组 $$A$$ 存储一个普通 $$m \times n$$ 矩阵，假设将矩阵转置结果存储到 $$n \times m$$ 矩阵 $$B$$ 中
...
上述程序段需要访问矩阵中的每一个元素，时间复杂度为 $$O(m \times n)$$

**稀疏矩阵转置算法-1**

步骤1：依次访问 $$A$$ 的行三元组表中各个三元组 $$<i,j,a_{ij}>$$ ，交换元素行列号后依次保存到 $$B$$ 的行三元组表中

步骤2：要满足规则，**行号从小到大排列，行号相同的列号从小到大排列**

![](./image/sz9.png)

步骤1的时间复杂度为 $$O(t)$$

步骤2是一个排序过程，采用不同排序算法的时间复杂度为 $$O(t^2)$$ 或 $$O(tlog_2t)$$

**稀疏矩阵转置算法-2**

步骤1：对 $$A$$ 的行三元组表进行第 $$1$$ 次扫描，找到列下标 $$i=0$$ 的所有三元组 $$<i,0,a_{i0}>$$ ，交换元素行列号后**依次**保存到 $$B$$ 的行三元组表中

步骤2：对 $$A$$ 的行三元组表进行第 $$2$$ 次扫描，找到列下标 $$i=1$$ 的所有三元组 $$<i,1,a_{i1}>$$ ，交换元素行列号后依次保存到 $$B$$ 的行三元组表中

步骤n：对 $$A$$ 的行三元组表进行第 $$n$$ 次扫描，找到列下标 $$i=n-1$$ 的所有三元组 $$<i, n-1, a_{i,n-1}>$$ ，交换元素行列号后依次保存到 $$B$$ 的行三元组表中
...
- 数组 $$num$$ 的数组元素 $$num[i]$$ 统计稀疏矩阵 $$A$$ 中列号为 $$i$$ 的非零元个数
...
<img src="./image/sz12.png" style="zoom:67%;" />

数组 $$k$$ 中 $$k[j]$$ 统计稀疏矩阵 $$A$$ 中列号从 $$0$$ 到 $$j-1$$ 列(前 $$j$$ 列)的非零元个数总和

$$k[j]$$ 表示 $$A$$ 中 $$j$$ 列第一个非零元在转置稀疏矩阵 $$B$$ 的行三元组表中的下标

![](./image/sz13.png)

- $$k[j]$$ 的值表示稀疏矩阵列号为的列中第一个非零元在转置矩阵中的存储位置
- $$k[j]$$ 每被访问一次执行一次自加操作，暗示该列中下一个非零元的存储位置
- 只需要对稀疏矩阵A的行三元组表进行一遍扫描，时间复杂度为 $$O(t)$$
- 稀疏矩阵的快速转置矩阵时间复杂度为 $$O(n+t)$$ (包含辅助数组的计算时间)
...
存储每个元素之后，对K进行自加操作，告诉下一个元素应该放在哪

![](./image/sz17.png)

### 例题

（1）**将7x8的二维数组A按照行优先顺序存储到一维数组B中，则B[42]中存储的二维数组元素是？**

公式： $$k=i×8+j$$

对 $$k=42$$ ：

 $$i=42\div 8=5$$ ，$$j=42\bmod 8=2$$

 所以 `B[42]` 存的是 `A[5][2]`

（2）**10x10的整型数组A，其每个数组元素占2个字节，已知 `A[0][0]` 在内存中的地址是100，按行优先，`A[4][6]`的地址是？**

按**行优先**，地址计算公式（下标从0开始）： 

$$
\text{addr}(A[i][j]) = \text{addr}(A[0][0]) + (i \times 10 + j)\times 2
$$

代入 $i=4,\ j=6$：

- 元素序号偏移： $$4\times 10 + 6 = 46$$
- 字节偏移： $$46 \times 2 = 92$$
- 地址： $$100 + 92 = 192$$

所以 `A[4][6]` 的地址是 192

（3）**设有6阶对称矩阵A，其中矩阵元素用a(i,j)表示，i为行下标i=0,1……,n-1 ，j为列下标，j=0,1.….,n-1，将A按照行优先顺序存储下三角元素的方式存储至一维数组B，设每个矩阵元素占2个字节，已知数组B的首地址为100，则，a(1,3)的地址是？**

因为是 **6 阶对称矩阵**，且只按**行优先存下三角元素（含主对角）**，所以：

- $a(1,3)$ 在上三角（ $$3>1$$ ），实际存的是对称位置 **$a(3,1)$**

下三角按行优先映射到一维数组下标（从 0 开始）：

$$
\text{pos}(i,j)=\frac{i(i+1)}{2}+j \quad (i\ge j)
$$

对 $a(3,1)$：

$$
\text{pos}=\frac{3\cdot 4}{2}+1=6+1=7
$$

每元素 2 字节，B 首地址 100：

$$
\text{addr}=100 + 7\times 2 = 114
$$

（3）**在有序表8,10,22,29,41,59,61,80,86,92上查找元素10，若执行对半搜索，需要比较几次查找成功？**

1. 第一次比较：中间元素是 **41**（第5个），10 < 41
2. 第二次比较：在前半段中间元素是 **10**，查找成功
...
高度为 $$h$$ 的二叉树上至多有 $$2^h-1$$ 个结点

具有 $$n$$ 个结点的完全二叉树的高度为 $$\lceil log_2(n+1) \rceil$$

任一棵二叉树中，若叶结点数量为 $$n_0$$ ，度为 $$2$$ 的结点数量为 $$n_2$$ ，则有： $$n_0=n_2+1$$

满二叉树：高度为 $$h$$ ，且有 $2^h-1$ 个结点的二叉树

包括 $$n$$ 个结点二叉树的高度最矮为 $$\lceil log_2(n+1) \rceil$$ ，最高为 $$n$$
...
int BinarySearch(ListSet L, ElemType x, int low, int high){
    if(low <= high){
        int m = (low + high) / 2;
        if(L.element[m] > x)
            return BinarySearch(L, x, low, m - 1); 
        // 中点比x大，说明x在左半边
        else if(L.element[m] < x)
            return BinarySearch(L, x, m + 1, high);
        // 中点比x小，说明x在右半边
        else
            return m;
    }
...
（1）**若一棵完全二叉树有17个结点，则该二叉树的叶子结点数量是？**

完全二叉树有 $n=17$ 个结点时，叶子结点数
$$
L=\left\lceil \frac{n}{2}\right\rceil=\left\lceil \frac{17}{2}\right\rceil=9
$$
（2）**一个二叉树上有度为0的结点有2个，度为1的结点有5个，则该树结点总数为？**

树中边数满足：
$$
n-1 = 1\cdot n_1 + 2\cdot n_2
$$
且
$$
n = n_0+n_1+n_2
...
- 根是 **A**
- A 的左子树中序是 **B** ⇒ 左孩子是 **B**
- A 的右子树中序是 **C E D**，右子树先序是 **C D E**
  - 右子树根 **C**，其右子树中序 **E D**，先序 **D E**
    - 根 **D**，左孩子 **E**

层次遍历（从上到下、从左到右）为：**A B C D E**
